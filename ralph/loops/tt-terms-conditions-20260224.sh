...
# Ralph loop for: tt-terms-conditions-20260224
# Terms and Conditions for Kari Time

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-terms-conditions-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-terms-conditions-20260224.sh"
}
...
info "Creating Terms and Conditions for Kari Time..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Terms and Conditions complete at iteration $ITERATION"
    exit 0
  fi
