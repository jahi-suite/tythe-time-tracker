...
# Ralph loop for: tt-logged-in-as-empty-20260224
# Fix "Logged in as: ()" empty user display

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-logged-in-as-empty-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-logged-in-as-empty-20260224.sh"
}
...
info "Fixing logged-in-as-empty user display..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Logged-in-as-empty fix complete at iteration $ITERATION"
    exit 0
  fi
