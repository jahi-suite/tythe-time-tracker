...
# Ralph loop for: tt-marketing-pdf-v2-20260224
# Sync PDF to v2 marketing page. Run AFTER tt-marketing-page-v2-20260224.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-marketing-pdf-v2-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-marketing-pdf-v2-20260224.sh"
}
...
info "Syncing PDF to v2 marketing page (run page v2 first if needed)..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Marketing PDF v2 complete at iteration $ITERATION"
    exit 0
  fi
