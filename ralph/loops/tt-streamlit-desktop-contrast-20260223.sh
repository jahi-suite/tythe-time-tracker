...
# Ralph loop for: tt-streamlit-desktop-contrast-20260223
# Fix Streamlit desktop contrast (selectboxes, buttons, form controls)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-streamlit-desktop-contrast-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-streamlit-desktop-contrast.sh"
}
...
info "Fixing Streamlit desktop contrast..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Desktop contrast fix complete at iteration $ITERATION"
    exit 0
  fi
