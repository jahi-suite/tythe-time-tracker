...
# Ralph loop for: tt-ts-app-dashboard-style-20260223
# Apply Manage Users dashboard style across whole tt-ts app

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-ts-app-dashboard-style-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-ts-app-dashboard-style.sh"
}
...
info "Applying dashboard style across tt-ts app..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "App-wide dashboard style complete at iteration $ITERATION"
    exit 0
  fi
