...
# Ralph loop for: tt-logout-and-unknown-user-20260224
# Fix logout destination and stricter unknown-user rejection

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-logout-and-unknown-user-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-logout-and-unknown-user-20260224.sh"
}
...
info "Fixing logout and unknown-user..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Logout and unknown-user fix complete at iteration $ITERATION"
    exit 0
  fi
