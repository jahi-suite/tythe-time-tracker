...
# Ralph loop for: tt-break-deduction-20260223
# 20-minute unpaid break for 6+ hour shifts (deduct from Standard, never Enhanced)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-break-deduction-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-break-deduction.sh"
}
...
info "Implementing 6-hour break deduction..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Break deduction complete at iteration $ITERATION"
    exit 0
  fi
