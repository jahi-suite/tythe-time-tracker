...
# Ralph loop for: tt-login-auth-diagnosis-20260224
# Diagnose and fix login/auth failure (Unknown user, session not persisting)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-login-auth-diagnosis-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-login-auth-diagnosis-20260224.sh"
}
...
info "Diagnosing login/auth failure..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Login/auth diagnosis complete at iteration $ITERATION"
    exit 0
  fi
