...
# Ralph loop for: tt-venue-export-delete-20260226
# Venue settings: export account data and delete account buttons.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-venue-export-delete-20260226"
TASK_DIR="docs/working-memory/open/$TASK_ID"
PROMPT_FILE="$REPO_ROOT/ralph/prompts/$TASK_ID.md"

export RALPH_BACKEND="${RALPH_BACKEND:-claude}"
MAX_ITERATIONS="${RALPH_MAX_ITERATIONS:-30}"
SLEEP="${RALPH_SLEEP:-3}"

verify() {
  "$REPO_ROOT/ralph/verify-tt-venue-export-delete-20260226.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Venue export/delete complete at iteration $ITERATION"
    exit 0
  fi
