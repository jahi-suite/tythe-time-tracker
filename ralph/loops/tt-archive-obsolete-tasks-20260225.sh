...
# Ralph loop for: tt-archive-obsolete-tasks-20260225
# Archive obsolete Netlify/legacy tasks to done/

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-archive-obsolete-tasks-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-archive-obsolete-tasks-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Archive complete at iteration $ITERATION"
    exit 0
  fi
