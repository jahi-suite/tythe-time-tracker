...
# Ralph loop for: tt-app-ui-20260225
# App UI fixes: venue login style, duplicate tabs, pay rates info box

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-app-ui-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-app-ui-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "App UI complete at iteration $ITERATION"
    exit 0
  fi
