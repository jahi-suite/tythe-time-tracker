...
# Ralph loop for: tt-venue-supervisor-settings-20260225
# Per-venue supervisor settings — UI, Clock, Manager, export

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-venue-supervisor-settings-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-venue-supervisor-settings-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Venue supervisor settings complete at iteration $ITERATION"
    exit 0
  fi
