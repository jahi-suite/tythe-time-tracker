...
# Ralph loop for: tt-netlify-502-debug-20260224
# Fix Netlify 502 by using dynamic import in handler

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-netlify-502-debug-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-netlify-502-debug-20260224.sh"
}
...
info "Fixing Netlify 502 with dynamic import..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Netlify 502 fix complete at iteration $ITERATION"
    exit 0
  fi
