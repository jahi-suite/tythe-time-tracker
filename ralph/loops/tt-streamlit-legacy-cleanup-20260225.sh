...
# Ralph loop for: tt-streamlit-legacy-cleanup-20260225
# Remove legacy Streamlit Time architecture, unify DB access

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-streamlit-legacy-cleanup-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-streamlit-legacy-cleanup-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Streamlit legacy cleanup complete at iteration $ITERATION"
    exit 0
  fi
