...
# Ralph loop for: tt-cicd-github-gcp-20260224
# CI/CD: push to main → deploy to GCP Cloud Run

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-cicd-github-gcp-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-cicd-github-gcp-20260224.sh"
}
...
info "Setting up CI/CD (GitHub → Cloud Run)..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "CI/CD setup complete at iteration $ITERATION"
    exit 0
  fi
