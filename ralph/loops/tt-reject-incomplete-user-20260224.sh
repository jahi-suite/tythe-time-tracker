...
# Ralph loop for: tt-reject-incomplete-user-20260224
# Reject incomplete users (empty display_name and username)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-reject-incomplete-user-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-reject-incomplete-user-20260224.sh"
}
...
info "Rejecting incomplete users in getAuthUserById..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Reject incomplete user fix complete at iteration $ITERATION"
    exit 0
  fi
