...
# Ralph loop for: tt-contact-kari-links-20260225
# Contact email jahi@karisuite.com, Powered by Kari Suite → karisuite.com

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-contact-kari-links-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-contact-kari-links-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Contact and Kari links complete at iteration $ITERATION"
    exit 0
  fi
