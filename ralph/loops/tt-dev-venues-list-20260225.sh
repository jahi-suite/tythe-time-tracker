...
# Ralph loop for: tt-dev-venues-list-20260225
# Dev-only venues list: names, staff count, last used

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-dev-venues-list-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-dev-venues-list-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Dev venues list complete at iteration $ITERATION"
    exit 0
  fi
