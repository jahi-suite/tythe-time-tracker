...
# Ralph loop for: tt-admin-unknown-user-20260224
# Fix admin login showing Unknown user

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-admin-unknown-user-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-admin-unknown-user-20260224.sh"
}
...
info "Fixing admin Unknown user..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Admin Unknown user fix complete at iteration $ITERATION"
    exit 0
  fi
