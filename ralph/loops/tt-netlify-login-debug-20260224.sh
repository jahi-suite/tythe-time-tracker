...
# Ralph loop for: tt-netlify-login-debug-20260224
# Netlify login debug tooling

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-netlify-login-debug-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-netlify-login-debug-20260224.sh"
}
...
info "Adding Netlify login debug tooling..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Netlify login debug complete at iteration $ITERATION"
    exit 0
  fi
