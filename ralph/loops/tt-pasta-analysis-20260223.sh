...
# Ralph loop for: tt-pasta-analysis-20260223
# PASTA threat modeling — LEVEL UP security for pay-enabled app

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-pasta-analysis-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-pasta.sh"
}
...
info "PASTA threat modeling — LEVEL UP BROTHER!"
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "PASTA analysis complete at iteration $ITERATION"
    exit 0
  fi
