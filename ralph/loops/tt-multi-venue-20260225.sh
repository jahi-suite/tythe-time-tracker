...
# Ralph loop for: tt-multi-venue-20260225
# Multi-venue: sign-up, venue select, Tythe must keep working

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-multi-venue-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-multi-venue-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Multi-venue complete at iteration $ITERATION"
    exit 0
  fi
