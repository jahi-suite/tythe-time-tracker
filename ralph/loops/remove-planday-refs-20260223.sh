...
# Ralph loop for: remove-planday-refs-20260223
# Remove tt-ts-planday-style task references from codebase and git history

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="remove-planday-refs-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-remove-planday-refs.sh"
}
...
info "Removing tt-ts-planday-style task references..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Remove styling task refs complete at iteration $ITERATION"
    exit 0
  fi
