...
# Ralph loop for: tt-karisuite-contact-links-20260225
# Contact jahi@karisuite.com, Powered by → karisuite.com

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-karisuite-contact-links-20260225"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-karisuite-contact-links-20260225.sh"
}
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if verify; then
    ok "Kari Suite contact and links complete at iteration $ITERATION"
    exit 0
  fi
