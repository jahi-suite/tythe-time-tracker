...
# Ralph loop for: tt-ts-tabs-polish-20260223
# Fix Manager Dashboard tab visibility (inactive tabs invisible until hover)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-ts-tabs-polish-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-ts-tabs-polish.sh"
}
...
info "Fixing Manager Dashboard tab visibility..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Tabs polish complete at iteration $ITERATION"
    exit 0
  fi
