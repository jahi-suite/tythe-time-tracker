...
# Ralph loop for: tt-marketing-pdf-brochure-20260224
# Redesign marketing PDF as brochure-style

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-marketing-pdf-brochure-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-marketing-pdf-brochure-20260224.sh"
}
...
info "Redesigning marketing PDF as brochure..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Marketing PDF brochure complete at iteration $ITERATION"
    exit 0
  fi
