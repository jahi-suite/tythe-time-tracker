...
# Ralph loop for: tt-export-break-and-shift-validation-20260223
# Part A: Break clarity in exports | Part B: Add shift validation (existing user, case-insensitive)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-export-break-and-shift-validation-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-export-break-and-shift-validation.sh"
}
...
info "Implementing export break clarity + add shift validation..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Task complete at iteration $ITERATION"
    exit 0
  fi
