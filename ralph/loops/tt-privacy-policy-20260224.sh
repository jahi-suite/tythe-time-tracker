...
# Ralph loop for: tt-privacy-policy-20260224
# Privacy Policy for Kari Time

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-privacy-policy-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-privacy-policy-20260224.sh"
}
...
info "Creating Privacy Policy for Kari Time..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Privacy Policy complete at iteration $ITERATION"
    exit 0
  fi
