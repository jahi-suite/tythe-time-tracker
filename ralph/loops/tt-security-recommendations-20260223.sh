...
# Ralph loop for: tt-security-recommendations-20260223
# Implement PASTA security recommendations

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-security-recommendations-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-security.sh"
}
...
info "Implementing PASTA security recommendations..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Security recommendations complete at iteration $ITERATION"
    exit 0
  fi
