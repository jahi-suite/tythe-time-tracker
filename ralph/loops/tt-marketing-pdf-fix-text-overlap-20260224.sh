...
# Ralph loop for: tt-marketing-pdf-fix-text-overlap-20260224
# Fix text overlapping in marketing PDF

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-marketing-pdf-fix-text-overlap-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-marketing-pdf-fix-text-overlap-20260224.sh"
}
...
info "Fixing text overlap in marketing PDF..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Marketing PDF text overlap fixed at iteration $ITERATION"
    exit 0
  fi
