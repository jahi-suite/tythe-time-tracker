...
# Ralph loop for: tt-ts-manage-users-dashboard-20260223
# Refactor Manage Users tab into card-based dashboard

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-ts-manage-users-dashboard-20260223"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-ts-manage-users-dashboard.sh"
}
...
info "Refactoring Manage Users dashboard..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Manage Users dashboard complete at iteration $ITERATION"
    exit 0
  fi
