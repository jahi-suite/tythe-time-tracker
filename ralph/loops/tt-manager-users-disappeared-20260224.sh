...
# Ralph loop for: tt-manager-users-disappeared-20260224
# Fix users disappeared on manager tab

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
. "$REPO_ROOT/ralph/lib.sh"

TASK_ID="tt-manager-users-disappeared-20260224"
...
verify() {
  "$REPO_ROOT/ralph/verify-tt-manager-users-disappeared-20260224.sh"
}
...
info "Fixing users disappeared on manager tab..."
ITERATION=0
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ITERATION=$((ITERATION + 1))
  if verify; then
    ok "Manager users fix complete at iteration $ITERATION"
    exit 0
  fi
