...
# Environment: RALPH_BACKEND (gemini default|claude|cursor|codex|codex-cli), RALPH_MODEL (Cursor/Codex), RALPH_CLAUDE_MODEL (Claude), RALPH_GEMINI_MODEL (default: gemini-3-flash-preview)
  ...
  if [ "${RALPH_BACKEND:-gemini}" = "claude" ]; then
    ...
    (
      cd "$repo_root" && printf '%s' "$prompt_content" | $gemini_cmd --yolo \
        --model "${RALPH_GEMINI_MODEL:-gemini-3-flash-preview}"
    ) || true
  else
