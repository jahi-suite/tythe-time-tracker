...
- [x] break-clarity-03 — TypeScript Excel export: added `Break Deducted` column on Staff Hours & Shifts rows and footer note explaining the 20-minute unpaid break rule for 6h+ shifts.
...
- 2026-02-24: Completed `break-clarity-03` in `tt-ts/src/server/services/exportService.ts` (TypeScript Excel export). Staff Hours & Shifts rows now include a `Break Deducted` column based on gross pre-deduction shift hours (6h+ => `20 min`, otherwise `—`) and the sheet includes a break deduction note.
