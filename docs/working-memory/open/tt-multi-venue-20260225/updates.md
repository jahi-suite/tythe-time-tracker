...
- 2026-02-25 13:54 UTC: Completed `multi-venue-03` (auth venue scoping) in `tt-ts`: login now requires `venue_slug`/`venue_id`, resolves and scopes auth to the venue, and stores `venue_id` + `venue_slug` in session; `/api/auth/me` validates session venue before returning user; `first-setup` GET/POST validate venue existence and assign the created first manager to that venue; client auth requests now post `venue_slug` (default `tythe`) and support `/:venueSlug/login` routes for login/first-setup. Verified with `./ralph/verify-tt-multi-venue-20260225.sh`.
