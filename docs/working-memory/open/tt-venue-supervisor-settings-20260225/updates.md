...
- 2026-02-25T15:23:17+00:00: Completed `supervisor-05` (Clock page venue supervisor settings). `ClockPage` now fetches `/api/venues/current/settings`, defaults to existing behavior if fetch fails, hides the supervisor checkbox when `supervisor_enabled=false`, and uses `supervisor_label` for the Clock UI role label when shown.
