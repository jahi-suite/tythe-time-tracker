...
- break-05 verification run: `rg` confirms `apply_break_deduction` / `applyBreakDeduction` are present and used in Python + TypeScript call sites; `./ralph/verify-tt-break-deduction.sh` reaches and passes the `tt-ts` build step (`npm run build`). The final Python verification step fails in this shell because `pytest` is not installed (`python -m pytest` -> `No module named pytest`), so end-to-end verification remains environment-blocked.
