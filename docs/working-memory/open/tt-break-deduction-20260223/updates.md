...
- break-05 complete: End-to-end verification passed by running `PATH="$PWD/.venv/bin:$PATH" ./ralph/verify-tt-break-deduction.sh` (the shell lacks a global `python`, but the project venv provides `python`/`pytest`). Verified `rg` checks, `tt-ts` build (`npm run build`), and Python tests (`tests/unit/test_shift_splitting.py`) all pass.
