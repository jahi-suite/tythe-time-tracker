...
- 2026-02-25 14:57:40 GMT: Completed `rules-04` by threading per-venue settings through exports: `exportService` now loads session venue settings and passes them into `exportUtils` (`splitShiftByRate`, `applyBreakDeduction`, `calculateStaffSummary`) so export enhanced-hour splits follow venue-specific windows while defaults remain unchanged.
